...
class App extends Component {
  state = {
    ...
    // changeTimer: 0,
    isWon: true,
    firstThumbnail: imagesList[0].imageUrl,
  }
  ...
  checkValidation = id => {
    const {firstThumbnail} = this.state
    const firstThumbnailObject = imagesList.filter(
      eachImage => eachImage.imageUrl === firstThumbnail,
    )
    const firstThumbnailId = firstThumbnailObject[0].id

    if (firstThumbnailId !== id) {
      //   console.log(firstThumbnailObject, 'obj', id, firstThumbnailId)
      ...
      this.setState(prevState => ({
        ...
        firstThumbnail: imagesList[randomNumber].imageUrl,
      }))
    }
  }
  ...
  tick = () => {
    ...
    if (timer === 0) {
      this.setState(prevState => ({isWon: !prevState.isWon, timer: 0}))
      clearInterval(this.timerId)
    }
  }
  ...
  gameCard = () => {
    const {activeTabId, firstThumbnail} = this.state
    return (
      <div className="second-container">
        <div className="main-container">
          <img src={firstThumbnail} alt="match" className="thumbnail-img" />
        </div>
            ...
            {tabsList.map(eachTab => (
              <li key={eachTab.tabId}>
                <button
                  ...
                  type="button"
                  onClick={() => this.onClickChangeTab(eachTab.tabId)}
                >
                  {eachTab.displayText}
                </button>
              </li>
        ...
        <div className="ul-div">
          <ul className="ul-container ">
            {newImagesList.map(eachImage => {
              const {id} = eachImage

              return (
                <li key={id} onClick={() => this.checkValidation(id)}>
  ...
  render() {
    const {activeTabId, isWon} = this.state

    newImagesList = imagesList.filter(
    ...
    return (
      <div className="bg-container">
        ...
        {isWon ? this.gameCard() : this.resultCard()}
      </div>
    )
  }
}
